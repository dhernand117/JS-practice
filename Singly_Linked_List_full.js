...
//?The big O complexity of singly linked list is:
//? Insertion O(1)
//? Removal O(1) at the beginning O(n) if it is at the end because it has to go through all the nodes
//? Searching O(n)
//? Accessing O(n)
...
class SinglyLinkedList {
  ...
  push(val) {
    ...
    //*If the list has no elements this if statement will set the value to be the new head and tail
    if (!this.head) {
      this.head = newNode;
      this.tail = this.head;
  ...
  remove(index) {
    //*If the index is less than zero or greater than the length it will return undefined
  ...
  //*This method will reverse the order of the values of the linked list
  reverse() {
    //*This variable will initialize to become the head property of th elist
    var node = this.head;
    //*Swapping the va;ues of the head to be the tail
    this.head = this.tail;
    //*Finally pointing the tail to the node variable to complete the reverse method
    this.tail = node;
    //*This variables will serve as placeholders fr our nodes while we swap the direction inside the for loop
    var next;
    var prev = null;
    //*Loop that will reverse the direction of the nodes
    for (var i = 0; i < this.length; i++) {
      //*Setting next to be the next property on whatever node is (this will keep track of the value that followed the current node)
      next = node.next;
      //*Set the next property to be whathever prev is
      node.next = prev;
      //*Setting prev to be the value od the node variable
      prev = node;
      //*Setting the node variavle to be the value of the next variable
      node = next;
    }
    return this;
  }
  //*This method will log the values into an array to help us visualize the linked llist reversion
  print() {
    var arr = [];
    var current = this.head;
    while (current) {
